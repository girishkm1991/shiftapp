  ...
  public static async dispatch(payload: DispatchPayload): Promise<void> {
    ...
    try {
      ...
      for (const userId of recipients) {
        ...
        if (!user) {
          console.warn(`[NotificationDispatcherService] Recipient user with ID "${userId}" not found in database. Skipping.`);
          continue;
        }
        ...
        console.log('[NotificationDispatcher] Channel Preferences -> In-App:', inAppEnabled, 'Internal Msg:', internalMessagesEnabled, 'Telegram:', telegramEnabled);
      ...
      console.error('[NotificationDispatcherService] Fatal dispatch error:', err);
    }
  }
...
console.log('[NotificationDispatcherService] Diagnostics mode enabled.');
